...
// outgoing calls
export const StartAudioCall = (_id, id) => {
    return async (dispatch, getState) => {
        dispatch(slice.actions.resetAudioCallQueue()); // 1st resets audio call to make sure there's one call at a time
        axios
            .post(
                "/start-audio-call",
                { _id, id },
            ...
            .then((response) => {
                dispatch( // 2nd sets AudioCallQueue with the call object and incoming to false
...
// starting the call, when the call is declined or ends
